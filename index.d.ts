...
export class PageSystem {
    ...
    public embeds: MessageEmbed[] | null;
    ...
    constructor(client: Client) {
        ...
        this.footer = (index: number, total: number) => total - 1 ? `📜 Page ${index}/${total}` : '';
        this.filter = (i: ButtonInteraction) => this.id === i.user.id;
    }
    get ctx() {
    ...
    getOpts(disabled: boolean = false) {
        return {
            embeds: [this.embed()],
    ...
    isValidCtx(ctx: CommandInteraction | Message) {
        return [CommandInteraction, Message].some(c => ctx instanceof c);
    }
    checkForErrors(msg: CommandInteraction | Message) {
        let errors: string[] = []
        function add(error: string) { this.errors.push(error) };
    ...
    async start(ctx: CommandInteraction | Message) {
        ...
        this.collector.on('collect', (int:ButtonInteraction) => {
